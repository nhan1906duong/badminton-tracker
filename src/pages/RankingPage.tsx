...
import { BwfCategoryBadge, PullToRefresh, Tabs } from '../../design-system/components'
...
function TopOneWeekStreakText({ count }: { count: number }) {
  ...
  return (
    ...
    >
      ...
      <span className="inline min-[390px]:hidden">
        {t('ranking.topOneWeekStreakShort', { count })}
      </span>
    </div>
  )
}

function SessionPlayerRow({
  stat,
  rank,
  isLast,
  onClick,
...
export default function RankingPage() {
  ...
  const { data: completedMatchCount = 0, refetch: refetchCompletedMatchCount } =
    useCompletedMatchCount()
  ...
  const latestSession = useMemo(() => sessions.find(s => s.ended_at != null) ?? null, [sessions])
  ...
  useLayoutEffect(() => {
    if (activeTab === 'all' && allListRef.current) setAllScrollMargin(allListRef.current.offsetTop)
        ...
        <div className="min-h-svh pb-24" style={{ background: 'var(--bg)' }}>
          ...
          >
            ...
            {!isLoading && rankings.length > 0 && (
              ...
              >
                {t('units.player', { count: playerCount })} ·{' '}
                {t('units.match', { count: completedMatchCount })}
              </p>
            )}
          </div>
          ...
          {activeTab === 'all' ? (
            isLoading ? (
              ...
              <div
                ...
                style={{
                  position: 'relative',
                  height: allVirtualizer.getTotalSize(),
                          ...
                          {!isNew && s.lastSessionRatingDelta !== 0 && (
                            <div
                              style={{
                                ...
                                fontVariantNumeric: 'tabular-nums',
                                color:
                                  s.lastSessionRatingDelta > 0 ? 'var(--success)' : 'var(--danger)',
                              }}
                            >
                              {s.lastSessionRatingDelta > 0 ? '+' : ''}
                              {s.lastSessionRatingDelta}
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          {formatShortPlayerName(pair.player1.name)} /{' '}
                          {formatShortPlayerName(pair.player2.name)}
                        </div>
                        <PlayerRecordLine
