...
function buildSeedLog(a: number, b: number): LogEntry[] {
  ...
  while (ca < a || cb < b) {
    if (ca < a && (cb >= b || Math.random() < 0.6)) {
      ca++
      log.push({ team: 'A', a: ca, b: cb })
    } else if (cb < b) {
      cb++
      log.push({ team: 'B', a: ca, b: cb })
    }
  }
  return log
}
...
function liveReducer(state: LiveState, action: LiveAction): LiveState {
  switch (action.type) {
    ...
    case 'increment': {
      ...
      if (action.team === 'A') next.a++
      else next.b++
      return {
        score: next,
        serving: action.team,
        log: [...state.log, { team: action.team, a: next.a, b: next.b }],
      }
    }
    case 'decrement': {
...
export default function MatchDetailPage() {
  ...
  const [live, dispatch] = useReducer(liveReducer, {
    score: { a: 0, b: 0 },
    serving: null,
    log: [],
  })
  ...
  const completedWinnerLabel = isCompleted
    ? (match.teams.find(t => t.is_winner)?.team_label ?? null)
    : null
  ...
  const kpCanSave =
    kpEntered !== null && kpEntered >= 0 && kpEntered <= HARD_CAP && kpEntered !== kpCurrentScore

  // ── Sub-renders ───────────────────────────────────────────────────────────

  const KP_KEYS: Array<string | 'back' | 'clear'> = [
    '1',
    '2',
    '3',
    '4',
          ...
          >
            <span
              style={{
                ...
                fontWeight: 800,
                color: 'var(--surface)',
                background: kpTeam === 'A' ? 'var(--fg)' : 'var(--accent)',
              }}
            >
              {kpTeam}
            </span>
        ...
        >
          <span
            style={{
              ...
              lineHeight: 0.9,
              color: kpEntered !== null ? 'var(--fg)' : 'var(--muted)',
              opacity: kpEntered !== null ? 1 : 0.4,
              fontVariantNumeric: 'tabular-nums',
              minWidth: '2ch',
            }}
          >
              ...
              <>
                ...
                <span
                  style={{
                    ...
                    letterSpacing: '-0.02em',
                    marginTop: 2,
                    color: kpDelta > 0 ? 'var(--accent)' : 'var(--muted)',
                  }}
                >
                  {kpDelta > 0 ? '+' : ''}
                  {kpDelta}
        ...
        >
          {KP_KEYS.map((k, i) => (
            <button
              key={i}
              type="button"
              onClick={() => kpInput(k as string)}
              style={{
                background: k === 'back' || k === 'clear' ? 'var(--bg)' : 'var(--surface)',
                border: '1px solid var(--border)',
                borderRadius: 'var(--radius-lg)',
                padding: 'var(--space-3)',
                fontFamily:
                  k === 'back' || k === 'clear' ? 'var(--font-body)' : 'var(--font-display)',
                fontSize: k === 'back' || k === 'clear' ? 'var(--text-sm)' : 'var(--text-xl)',
                fontWeight: 700,
                color: k === 'back' || k === 'clear' ? 'var(--muted)' : 'var(--fg)',
                cursor: 'pointer',
                minHeight: 56,
                display: 'grid',
                placeItems: 'center',
                letterSpacing: '-0.02em',
                textTransform: k === 'clear' ? 'uppercase' : undefined,
                touchAction: 'manipulation',
              }}
            >
              {k === 'back' ? (
        ...
        <div style={{ display: 'flex', gap: 'var(--space-2)', padding: '0 var(--space-2)' }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 'var(--text-base)',
              fontWeight: 600,
              cursor: kpCanSave ? 'pointer' : 'not-allowed',
              minHeight: 48,
              border: 'none',
              background: kpCanSave ? 'var(--accent)' : 'var(--border)',
              color: kpCanSave ? 'var(--surface)' : 'var(--muted)',
              opacity: kpCanSave ? 1 : 0.6,
              touchAction: 'manipulation',
            }}
          >
        ...
        >
          {(['A', 'B'] as const).map(team => (
            <button
              key={team}
              type="button"
              onClick={() => handleFinalizeWin(team)}
              style={{
                display: 'flex',
            ...
            >
              <span
                style={{
                  ...
                  fontWeight: 800,
                  color: 'var(--surface)',
                  background: team === 'A' ? 'var(--fg)' : 'var(--accent)',
                  marginBottom: 2,
                }}
              >
                {team}
      ...
      <>
        {isLive && (
          <>
            <BottomSheetItem
              ...
              label={t('matchDetail.enterScoreDirectly')}
              onClick={() => {
                setSheet(null)
                setTimeout(() => openScoreEdit('A'), 300)
              }}
            />
            <BottomSheetItem
              icon={<CheckCircle size={20} />}
              label={t('matchDetail.recordTeamWin')}
              onClick={() => setSheet('award')}
            />
            <BottomSheetItem
              icon={<ArrowLeftRight size={20} />}
              label={t('matchDetail.swapServe')}
              onClick={() => {
                setSheet(null)
                swapServe()
              }}
            ...
            <BottomSheetItem
              ...
              label={t('matchDetail.undoLastPoint')}
              onClick={() => {
                setSheet(null)
                undoLast()
              }}
        ...
        {isScheduled && (
          <BottomSheetItem
            ...
            label={t('matchDetail.startMatchMenu')}
            onClick={() => {
              setSheet(null)
              handleStartMatch()
            }}
        ...
        {isCompleted && (
          <BottomSheetItem
            ...
            label={t('matchDetail.reopenMatch')}
            onClick={() => {
              setSheet(null)
              handleReopenMatch()
            }}
        ...
        <BottomSheetItem
          icon={<Pencil size={20} />}
          label={t('matchDetail.editPlayers')}
          onClick={() => navigate(`/sessions/${sessionId}/matches/${matchId}/players/edit`)}
        />
        <BottomSheetDivider />
        {isLive && (
          <BottomSheetItem
            icon={<Square size={20} />}
            label={t('matchDetail.endMatch')}
            onClick={() => {
              setSheet(null)
              setConfirmEndNoWinnerOpen(true)
            }}
            danger
          />
        )}
        {isAdmin && (
          <BottomSheetItem
            icon={<Trash2 size={20} />}
            label={t('matchDetail.deleteMatch')}
            onClick={() => {
              setSheet(null)
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          paddingBottom: 'max(140px, calc(env(safe-area-inset-bottom) + 120px))',
        }}
      >
        {/* Hero */}
        <header style={{ padding: 'var(--space-3) var(--space-5) var(--space-5)' }}>
          ...
          <div
            style={{
              ...
              marginBottom: 'var(--space-3)',
              minHeight: 18,
              color: isLive ? 'var(--accent)' : isScheduled ? 'var(--muted)' : 'var(--fg)',
            }}
          >
            {isLive && (
              <span
          ...
          >
            ...
            {isScheduled ? (
              ...
              <>
                <span>
                  {t('matchDetail.startedAt', { time: formatTime(match.played_at, locale) })}
                </span>
                <span
                  style={{
                    width: 3,
                ...
                <span>
                  {isLive
                    ? t('units.elapsed', { duration: getElapsed(match.played_at) })
                    : t('matchDetail.completed')}
                </span>
              </>
            )}
          </div>
            ...
            >
              {(['A', 'B'] as const).map(team => (
                <button
                  ...
                  type="button"
                  role="radio"
                  aria-checked={serving === team}
                  onClick={() => dispatch({ type: 'set_serving', serving: team })}
                  style={{
                    background: serving === team ? 'var(--accent-soft)' : 'var(--bg)',
                    border:
                      serving === team ? '2px solid var(--accent)' : '1px solid var(--border)',
                    borderRadius: 'var(--radius-md)',
                    padding: serving === team ? 'calc(var(--space-3) - 1px)' : 'var(--space-3)',
                    fontFamily: 'var(--font-body)',
                    fontSize: 'var(--text-sm)',
                    fontWeight: 600,
                    color: serving === team ? 'var(--accent)' : 'var(--fg)',
                    cursor: 'pointer',
                    minHeight: 44,
                    display: 'inline-flex',
                    flexDirection: 'column',
                ...
                >
                  <span
                    style={{
                      ...
                      textTransform: 'uppercase',
                      letterSpacing: '0.08em',
                      color: serving === team ? 'var(--accent)' : 'var(--muted)',
                    }}
                  >
                    {t('team.servesFirst')}
                  </span>
        ...
        {(isLive || isCompleted) && (
          <>
            ...
            >
              <div
                style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', position: 'relative' }}
              >
                {/* Center divider */}
                <div
                  aria-hidden
                ...
                {(['A', 'B'] as const).map(team => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        style={{
                          alignSelf: team === 'A' ? 'flex-start' : 'flex-end',
                          display: 'inline-flex',
                          alignItems: 'center',
                          gap: 5,
                          fontFamily: 'var(--font-mono)',
                          ...
                          letterSpacing: '0.1em',
                          color: 'var(--accent)',
                          visibility: isServingNow ? 'visible' : 'hidden',
                      ...
                      <span
                        style={{
                          ...
                          fontSize: 'var(--text-base)',
                          letterSpacing: '-0.01em',
                          color: isLoser ? 'var(--muted)' : 'var(--fg)',
                          lineHeight: 1.2,
                          minHeight: 38,
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                          fontWeight: isLoser ? 500 : 700,
                        }}
                      >
                        {name}
                      </span>
                        ...
                        style={{
                          ...
                          color: isWinner
                            ...
                            : isLoser
                              ...
                              : 'var(--fg)',
                          animation: isBumped
                            ? 'score-bump 0.32s cubic-bezier(0.32, 0, 0.15, 1)'
                            : undefined,
                          position: 'relative',
                        }}
                      >
                        ...
                        {isLive && (
                          <span
                            ...
                            style={{
                              ...
                              right: 8,
                              bottom: 0,
                              borderBottom: '1.5px dotted var(--border)',
                              pointerEvents: 'none',
                            }}
                          />
                        )}
                      ...
                      >
                        <button
                          ...
                          disabled={score === 0 || !isLive}
                          aria-label={t('matchDetail.subtractOne', { team })}
                          style={{
                            background: 'var(--bg)',
                            border: '1px solid var(--border)',
                            width: 28,
                            ...
                            color: 'var(--muted)',
                            borderRadius: 'var(--radius-md)',
                            cursor: score === 0 || !isLive ? 'not-allowed' : 'pointer',
                            opacity: score === 0 || !isLive ? 0.35 : 1,
                            touchAction: 'manipulation',
                          }}
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => openScoreEdit(team)}
                          disabled={!isLive}
                          aria-label={t('matchDetail.enterScoreDirectly')}
                          style={{
                            background: 'var(--bg)',
                            border: '1px solid var(--border)',
                            width: 28,
                      ...
                      {isCompleted && isWinner ? (
                        ...
                        <span style={{ visibility: 'hidden', marginTop: 'auto', fontSize: 10 }}>
                          .
                        </span>
                      )}
                    </div>
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: 999,
                    width: `${pct}%`,
                    transition: 'width 0.3s cubic-bezier(0.32, 0, 0.15, 1)',
                  }}
                />
              </div>
              <span
                  ...
                  <>
                    ...
                    >
                      {reopenMatch.isPending ? (
                        ...
                        <RefreshCw style={{ width: 16, height: 16, color: 'var(--muted)' }} />
                      )}
                    ...
                    {completedWinnerLabel && (
                      <button
                        type="button"
                        onClick={() => navigate(`/sessions/${sessionId}/matches/${matchId}/points`)}
                        style={{
                          background: 'var(--surface)',
                          border: '1px solid var(--border)',
                          borderRadius: 'var(--radius-lg)',
                  ...
                  <>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        fontWeight: 600,
                        color: 'var(--fg)',
                        cursor: pointLog.length === 0 ? 'not-allowed' : 'pointer',
                        display: 'inline-flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        gap: 'var(--space-2)',
                        minHeight: 48,
                        opacity: pointLog.length === 0 ? 0.4 : 1,
                        touchAction: 'manipulation',
                      }}
                    >
      ...
      >
        {isScheduled && user && (
          <button
            ...
            disabled={!serving || startMatch.isPending}
            style={{
              width: '100%',
              padding: 'var(--space-4)',
              background: serving ? 'var(--accent)' : 'var(--border)',
              color: serving ? 'var(--surface)' : 'var(--muted)',
              fontFamily: 'var(--font-body)',
              fontSize: 'var(--text-base)',
              fontWeight: 700,
              border: 'none',
          ...
          >
            ...
            {startMatch.isPending
              ? t('matchDetail.starting')
              : serving
                ? t('matchDetail.startMatch')
                : t('matchDetail.pickServeFirst')}
            {serving && !startMatch.isPending && (
              <span
                style={{
                  fontFamily: 'var(--font-mono)',
                  fontSize: 'var(--text-xs)',
          ...
          (() => {
            ...
            if (autoWinner) {
              return (
                <button
                  type="button"
                  onClick={() => handleFinalizeWin(autoWinner)}
            ...
            return (
              <>
                ...
                <button
                  type="button"
                  onClick={() => setConfirmEndNoWinnerOpen(true)}
                  style={{
                    width: '100%',
                    padding: 'var(--space-4)',
                    background: 'var(--surface)',
      ...
      <Dialog
        ...
        description={t('matchDetail.endNoWinnerDescription', {
          score: `${liveScore.a}–${liveScore.b}`,
        })}
        kind="warning"
        actions={[
          {
            ...
            variant: 'secondary',
            onClick: () => setConfirmEndNoWinnerOpen(false),
          },
          {
      ...
      <Dialog
        ...
        title={
          isLive || isCompleted
            ? t('matchDetail.deleteRecordedTitle')
            : t('matchDetail.deleteTitle')
        }
        description={
          isLive || isCompleted
            ? t('matchDetail.deleteRecordedDescription', {
                number: matchNumber,
                teamA: teamAName,
                teamB: teamBName,
              })
            : t('matchDetail.deleteDescription', {
                number: matchNumber,
                teamA: teamAName,
                teamB: teamBName,
              })
