...
const LeagueTeamBuilder = forwardRef<LeagueTeamBuilderHandle, LeagueTeamBuilderProps>(
  function LeagueTeamBuilder({ teams, matchType, onChange, showShuffleButton = true }, ref) {
    const { t } = useI18n()
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        {!allTeamsValid && teams.length >= 2 && (
          <p
            className="font-[family:var(--font-mono)] text-[var(--danger)]"
            style={{ fontSize: 12 }}
          >
          ...
          >
            {filteredPlayers.length === 0 ? (
              ...
              filteredPlayers.map(p => {
                ...
                const isTeamFull =
                  pickerTeamIndex !== null &&
                  (teams[pickerTeamIndex]?.playerIds.length ?? 0) >= requiredPerTeam
                return (
                  <button
                    key={p.id}
                    type="button"
          ...
          <div style={{ padding: '0 var(--space-5) var(--space-3)' }}>
            ...
            >
              ...
              >
                {allPlayers &&
                allPlayers.length > 0 &&
                allPlayers.every(player => shuffleSelectedIds.has(player.id))
                  ? t('shuffle.clearAll')
                  : t('shuffle.selectAll')}
              </button>
            </div>
