import { Calendar, ChevronRight, List, Zap } from 'lucide-react'
import { SectionLabel, SegmentedControl } from '../../../design-system/components'
...
import { friendlyDate, friendlyTime, roundedSoon, toDateInput, toTimeInput } from './helpers'
      ...
      <div style={{ marginBottom: 'var(--space-4)' }}>
        <SegmentedControl
          ...
          tabs={[
            {
              id: 'now' as const,
              label: t('createMatch.now'),
              icon: <Zap style={{ width: 13, height: 13 }} />,
            },
            {
              id: 'schedule' as const,
              label: t('createMatch.schedule'),
              icon: <Calendar style={{ width: 13, height: 13 }} />,
            },
            {
              id: 'queue' as const,
              label: t('createMatch.queue'),
              icon: <List style={{ width: 13, height: 13 }} />,
            },
          ]}
        />
      </div>
            ...
            <div>
              ...
              >
                {scheduledMatchesCount > 0
                  ? t('createMatch.afterQueued', { count: scheduledMatchesCount })
                  : t('createMatch.nextToPlay')}
              </div>
              <div
                style={{
                  fontSize: 'var(--text-xs)',
              ...
              >
                ...
                <span style={{ flex: 1, color: 'var(--accent)', fontWeight: 700 }}>
                  {liveMatch.participants
                    .filter(
                      p => p.team_id === liveMatch.teams.find(t => t.team_label === 'TEAM_A')?.id,
                    )
                  ...
                  {liveMatch.participants
                    .filter(
                      p => p.team_id === liveMatch.teams.find(t => t.team_label === 'TEAM_B')?.id,
                    )
                    .map(p => formatShortPlayerName(p.player.name))
                    .join(' + ')}
                </span>
